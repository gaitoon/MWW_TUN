...
function mpSelRamiTarget(btn){
  sfxClick();
  document.querySelectorAll('#mpRamiTargetWrap .round-px-btn').forEach(b=>b.classList.remove('sel'));
  btn.classList.add('sel');
    ...
    if(G.gameType === 'rami'){
      ...
      switch(inp.action){
        ...
        case 'place':{
          ...
          ramiPlayerPlaceOnTable(inp.meldIndex);
          break;
        }
      }
...
<div style="display:flex;flex-direction:column;flex:1;min-height:0;gap:clamp(4px,0.8vh,8px);">
  ...
  <div style="display:flex;justify-content:center;flex-shrink:0;">
    <div style="display:inline-flex;align-items:center;gap:clamp(6px,1vw,10px);padding:clamp(4px,0.8vh,6px) clamp(12px,2vw,18px);background:rgba(0,0,0,0.85);border:3px solid #8B6914;box-shadow:2px 2px 0 #000;">
      ...
      <span style="font-family:'Press Start 2P',monospace;font-size:clamp(7px,1vw,10px);color:#C9A84C;" id="ramiAiName">SAMI</span>
...
function ramiPlayerDraw(){
  // In MP: only the host runs this directly (for host's own draw).
  // Guests send rami_input and wait for snapshot — never mutate state locally.
  if(G.mp && !MP.isHost) return;
  const mySlot = G.mp ? MP.mySlot : 0;
  if(G.busy||G.rami.currentTurn!==mySlot||G.rami.playerPhase!=='draw')return;
  const rami=G.rami;
  const ban=rami.banRounds&&rami.banRounds[mySlot]||0;
  if(ban>0){
    rami.banRounds[mySlot]--;
    renderRamiGame();
    ramiSetMsg('BANNED — SKIPPING ('+(rami.banRounds[mySlot])+' LEFT)');
    if(G.mp && MP.isHost) MP.snapshotDirty=true;
    setTimeout(()=>{_ramiNextAiOrPlayer();},1100);
    return;
  }
...
function ramiPlayerDrawDiscard(){
  ...
  if(!canOpen(mm)){
    ...
    if(!rami.banRounds)rami.banRounds={};
    rami.banRounds[mySlot]=2;
    sfxLose();
    showToast('TOOK DISCARD BUT CANNOT OPEN — 2 TURN BAN!');
    ramiSetMsg('CANNOT OPEN — 2 TURN BAN!');
    renderRamiGame();
    if(G.mp && MP.isHost) MP.snapshotDirty=true;
    setTimeout(()=>{_ramiNextAiOrPlayer();},1700);
    return;
  }
  sfxCard();
  rami.playerPhase='play';
...
function ramiEndRound(wi){
  ...
  const gameOver=G.players.some(p=>p.score>=target);
  setTimeout(()=>{
    G.busy=false;
    if(gameOver){
      if(G.mp && MP.isHost) mpSendSnapshot({gameover:true});
...
function ramiNextRound(){
  ...
  // Sort hands — in MP, each player sees their own hand sorted; host sorts both
  ...
  if(G.mp){
    const mySlot=MP.mySlot;
    if(startTurn===mySlot){ramiSetMsg('ROUND '+G.round+' — YOUR TURN — DRAW A CARD');}
    else{ramiSetMsg('ROUND '+G.round+' — '+G.players[startTurn].name+' GOES FIRST...');}
    if(MP.isHost){MP.snapshotDirty=true;mpStartTimer();}
    return;
  }
  // Single-player AI
...
function ramiEndGame(){
  const sv=[...G.players].sort((a,b)=>a.score-b.score),wn=sv[0];
  sfxWin();launchConfetti();
...
function ramiApplyDraw(slot){
  ...
  const ban = rami.banRounds && rami.banRounds[slot] || 0;
  if(ban > 0){
    rami.banRounds[slot]--;
    renderRamiGame();
    ramiSetMsg(p.name + ' IS BANNED — SKIPPING (' + rami.banRounds[slot] + ' LEFT)');
    MP.snapshotDirty = true;
    setTimeout(()=>{ _ramiNextAiOrPlayer(); }, 1100);
    return;
  }
...
function ramiApplyDrawDiscard(slot){
  ...
  if(!canOpen(mm)){
    ...
    rami.discardPile.push(card);
    if(!rami.banRounds) rami.banRounds = {};
    rami.banRounds[slot] = 2;
    sfxLose();
    ramiSetMsg(p.name + ' TOOK DISCARD BUT CANNOT OPEN — 2 TURN BAN!');
    renderRamiGame();
    MP.snapshotDirty = true;
    setTimeout(()=>{ _ramiNextAiOrPlayer(); }, 1700);
    return;
...
function mpBeginRami(gamePlayers, ramiState, ramiTarget){
  ...
  setTimeout(()=>{
    ...
    if(startTurn === mySlot){
      ramiSetMsg('YOUR TURN — DRAW A CARD');
    } else {
      ramiSetMsg(G.players[startTurn].name + '\'S TURN...');
    }
