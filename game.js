...
function selectGame(game){
  ...
  if(game === '51'){
    ...
    // Show target score + player count + opening threshold for Rami, hide rounds
...
document.getElementById('btnStartGame').addEventListener('click',()=>{
  ...
  if(selectedGame === 'rami'){
    initRamiGame(selectedRounds, selectedDifficulty, selectedRamiPlayers||2);
    return;
  }
...
function init51Game(rounds, difficulty='medium'){
  ...
  G={
    ...
    players:[
      ...
      {id:1,name:'SAMI',score:0,hand:[],isAI:true},
      {id:2,name:'LAYLA',score:0,hand:[],isAI:true},
      {id:3,name:'KARIM',score:0,hand:[],isAI:true},
    ],
...
function initChkobbaGame(rounds, difficulty='medium'){
  ...
  G={
    ...
    players: is2v2 ? [
      ...
      {id:1,name:'LAYLA',score:0,hand:[],captured:[],chkobbas:0,isAI:true,team:2},     // Left
      {id:2,name:'SAMI',score:0,hand:[],captured:[],chkobbas:0,isAI:true,team:1},      // Top (Partner)
      {id:3,name:'KARIM',score:0,hand:[],captured:[],chkobbas:0,isAI:true,team:2},     // Right
    ] : [
      // 1v1 MODE - 2 players
      {id:0,name:'YOU',score:0,hand:[],captured:[],chkobbas:0,isAI:false},
      {id:1,name:'SAMI',score:0,hand:[],captured:[],chkobbas:0,isAI:true},
    ],
    teams: is2v2 ? {
  ...
  // Small delay to ensure game screen is loaded
  setTimeout(() => {
    ...
    <div style="display:flex;align-items:center;gap:20px;">
      <div style="display:flex;align-items:center;gap:12px;padding:8px 20px;
        ...
        <div style="font-family:'Press Start 2P',monospace;font-size:clamp(10px,1.5vw,14px);
          color:#C9A84C;" id="chkobbaOppName">SAMI</div>
    ...
    <div style="display:flex;align-items:center;gap:12px;">
      <div style="display:flex;align-items:center;gap:8px;padding:6px 16px;
        ...
        <div style="font-family:'Press Start 2P',monospace;font-size:clamp(9px,1.3vw,12px);
          color:#4CAF50;" id="chkobbaTopName">SAMI ★</div>
        <div style="font-family:'Press Start 2P',monospace;font-size:clamp(10px,1.5vw,14px);
  ...
  leftZone.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;padding:6px 16px;
      ...
      <div id="chkobbaLeftTurnDot" style="width:10px;height:10px;background:#4CAF50;
        border-radius:50%;display:none;box-shadow:0 0 10px #4CAF50;"></div>
      <div style="font-family:'Press Start 2P',monospace;font-size:clamp(9px,1.3vw,12px);
        color:#F44336;" id="chkobbaLeftName">LAYLA</div>
      <div style="font-family:'Press Start 2P',monospace;font-size:clamp(10px,1.5vw,14px);
  ...
  rightZone.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;padding:6px 16px;
      ...
      <div id="chkobbaRightTurnDot" style="width:10px;height:10px;background:#4CAF50;
        border-radius:50%;display:none;box-shadow:0 0 10px #4CAF50;"></div>
      <div style="font-family:'Press Start 2P',monospace;font-size:clamp(9px,1.3vw,12px);
        color:#F44336;" id="chkobbaRightName">KARIM</div>
      <div style="font-family:'Press Start 2P',monospace;font-size:clamp(10px,1.5vw,14px);
...
function startChkobbaRound(){
  ...
  G.lastCapture = null; // Reset last capture for new round
  hideOverlays();
  
  // Create and shuffle 40-card Italian deck (A,2,3,4,5,6,7,J,Q,K of 4 suits)
  G.deck = shuffle(createChkobbaDeck());
  
  // CHKOBBA RULE: Deal 3 cards to each player at start
  G.players.forEach(p => {
    p.hand = [];
    p.captured = []; // Clear captured cards from previous round
    for(let i = 0; i < 3; i++){
      if(G.deck.length > 0){
  ...
  // CHKOBBA RULE: Place exactly 4 cards face-up on the table
  G.table = [];
  for(let i = 0; i < 4; i++){
    if(G.deck.length > 0){
      G.table.push(G.deck.pop());
    }
  }
  
  // Determine starter (alternates each round)
  G.cur = G.round % 2 === 1 ? 0 : 1;
  
  G.busy = false;
  
  // MULTIPLAYER: Mark snapshot dirty so guest gets new round state
  if(G.mp && MP.isHost) MP.snapshotDirty = true;
  
  // Render appropriate UI (AFTER G.busy = false so cards are clickable)
  if(G.mode === '2v2'){
...
function playChkobbaCard(card, captureIndices = null){
  ...
  if(captureIndices && captureIndices.length > 0){
    ...
    if(has7ofDiamonds){
      ...
      showEl7ayaFlash();
      if(G.mp) G._showEl7aya = true; // Mark for guest
    }
    ...
    if(G.table.length === 0){
      ...
      showChkobbaFlash();
      if(G.mp) G._showChkobba = true; // Mark for guest
    }
...
// Show AI's played card and highlight captures
function showAICardReveal(card, captureIndices, callback){
  const gameScreen = document.getElementById('game');
  
  // Create card display in center
  ...
  // Remove and continue after delay
  setTimeout(() => {
...
function showRoundBanner(roundNum, totalRounds, onDone){
  const t = LANG[currentLang];
  ...
  numEl.textContent = t.round_banner + ' ' + roundNum + '/' + totalRounds;
  subEl.textContent = roundNum === 1 ? '★ ' + t.yourTurn.replace('!','') + ' ★' : 'GET READY!';
...
function humanClick(card){
  ...
  if(G.cur!==_mySlot||G.aiActive||G.busy||G.choosing)return;
  sfxHover();
  if(G.selectedCard&&G.selectedCard.r===card.r&&G.selectedCard.s===card.s){
    G.selectedCard=null;
  }else{
...
function renderGame(){
  ...
  myPlayer.hand.forEach((card,i)=>{
    ...
    div.className='hand-card'+(myTurn?'':' disabled')+(isNewDeal?' card-dealing deal-d'+i:'')+(isSelected?' selected':'');
    div.innerHTML='<div class="hc-rank '+col+'">'+card.r+'</div><div class="hc-suit '+col+'">'+card.s+'</div>'+(hint?'<div class="hc-hint">'+hint+'</div>':'');
    if(myTurn)div.addEventListener('click',()=>humanClick(card));
...
function mpSelGame(btn){
  sfxClick();
  document.querySelectorAll('#mpGameWrap .round-px-btn').forEach(b=>b.classList.remove('sel'));
  btn.classList.add('sel');
...
function mpSelTarget(btn){
  sfxClick();
  document.querySelectorAll('#mpTargetWrap .round-px-btn').forEach(b=>b.classList.remove('sel'));
  btn.classList.add('sel');
...
function mpSelRounds(btn){
  sfxClick();
  document.querySelectorAll('#mpRoundsWrap .round-px-btn').forEach(b=>b.classList.remove('sel'));
  btn.classList.add('sel');
...
function mpOnMsg(msg){
  ...
  switch(msg.type){
    ...
    case 'snapshot':
      ...
      try {
        ...
        if(msg.timerSlot !== undefined){
          ...
          if(msg.timerSlot !== prevSlot && !msg.loserSlot && !msg.gameover){
            MP._lastTimerSlot = msg.timerSlot;
            mpStartTimer(); // purely visual on guest — no auto-throw logic runs here
          }
        }
    ...
    case 'settings':
      ...
      if(!MP.isHost){
        document.getElementById('mpGuestGame').textContent = msg.game === '51' ? 'CINQUANTE ET UN' : 'CHKOBBA';
...
function mpStart51Game(){
  ...
  const aiNames = ['SAMI','LAYLA','KARIM'];
...
function mpBeginGame(gamePlayers, starter, totalRounds){
  ...
  setTimeout(()=>{
    renderGame();
    setMsg(G.players[starter].name+"'S TURN!");
    // MODIFIED: Only host runs AI and the authoritative tick loop.
    // Client only renders; it does NOT simulate.
    if(MP.isHost){
      if(G.players[starter].isAI) scheduleAI();
      mpStartTimer();
      mpStartHostTick();
...
function mpHostTick(){
  ...
  while(MP.inputQueue.length > 0){
    ...
    // Validate: only process if it's this player's turn and game isn't busy
    if(inp.slot !== G.cur){ continue; }
    if(G.busy){ continue; }
    if(G.gameType !== 'chkobba' && G.choosing){ continue; } // 51 specific

    // Find the card in the player's actual hand (prevents spoofed cards)
    const p = G.players[G.cur];
  ...
  // ADDED: Also send snapshot if MP.snapshotDirty was set by other host-side
  // mutations (round transitions, AI moves, score updates, etc.).
  if(MP.snapshotDirty){
    stateChanged = true;
    MP.snapshotDirty = false;
  }

  // MODIFIED: Only broadcast when something changed — not every tick.
  if(stateChanged){
    mpSendSnapshot();
  }
}
...
function mpSendSnapshot(extra={}){
  ...
  // Include lastMove data for opponent capture animations
  const lastMove = G._lastMove ? JSON.stringify(G._lastMove) : null;
  
  mpSend('snapshot', {
    state: JSON.stringify(snap),
    statusMsg: document.getElementById('msgStrip').textContent,
    timerSlot: G.cur,  // guest uses this to know whose turn timer is for
    lastMove: lastMove,  // send last move data
    ...extra
  });
  
  // Clear animation flags after sending (so they don't repeat)
  delete G._showEl7aya;
  delete G._showChkobba;
  delete G._lastMove;  // clear last move after sending
}
...
function mpStartTimer(){
  ...
  // Position timer above current player's zone
  const mySlot = MP.mySlot;
  const isMyTurn = (G.cur === mySlot);
  
  if(isMyTurn){
    // My turn - timer at BOTTOM
    bar.style.top = 'auto';
    bar.style.bottom = '0';
  } else {
    // Opponent's turn - timer at TOP
    bar.style.top = '0';
  ...
  // Capture whose turn it is NOW so we auto-throw the right player even if G.cur changes
  const timerSlot = G.cur;

  _timerIv=setInterval(()=>{
    sec--;
    fill.style.width=((sec/15)*100)+'%';
    if(sec<=5){ fill.style.background='#CC0000'; fill.classList.add('timer-danger'); }
    if(sec<=0){
      ...
      // Hide timer bar after animation
      setTimeout(() => {
        bar.style.display='none';
      }, 1500);

      // HOST handles auto-throw for any player (human or remote) whose turn timed out
      if(MP.isHost && G.cur===timerSlot && !G.busy && !G.choosing){
        ...
        if(hand.length>0){
          ...
          // Record idle turn — 2 in a row ends the game for that player
          mpRecordIdleTurn(G.cur);
          MP.inputQueue.push({slot:G.cur, card:card, choice:choice, tick:MP.seqOut});
        }
      }

      // CLIENT: if it's the local player's turn on a non-host client, send input to host
...
function isValidMeld(cards){
  ...
  // Check if this is a Q-K-A wrap sequence
  if(sv.length>=2){
    const hasQ=sv.some(c=>c.r==='Q');
    const hasK=sv.some(c=>c.r==='K');
    const hasA=sv.some(c=>c.r==='A');
    // Allow Q-K-A specifically (A at high end)
    if(hasQ&&hasK&&hasA&&sv.length===3&&jk.length===0){
      return sv[0].r==='A'&&sv[1].r==='Q'&&sv[2].r==='K';
    }
  }
...
function canReplaceJoker(card,meld){
  ...
  const trial=[...meld];trial[jkIdx]=card;return isValidMeld(trial)?jkIdx:-1;
}
...
function ramiAiTurn(){
  ...
  const banKey=aiId;
  // Check ban
  if(rami.banRounds&&rami.banRounds[banKey]>0){
    rami.banRounds[banKey]--;
    ramiSetMsg(ai.name+' BANNED — SKIPPING ('+rami.banRounds[banKey]+' LEFT)');
    setTimeout(()=>{_ramiNextAiOrPlayer();G.busy=false;},1000);return;
  }
  // Draw
  ...
  // Check discard-pile ban
  if(fromDiscard&&!rami.hasOpened[aiId]){const mm=bestMeldSet(ai.hand);if(!canOpen(mm)){if(drawn){ai.hand=ai.hand.filter(c=>!(c.r===drawn.r&&c.s===drawn.s));rami.discardPile.push(drawn);}if(rami.banRounds)rami.banRounds[banKey]=2;ramiSetMsg(ai.name+' TOOK DISCARD BUT CANNOT OPEN — BANNED 2 TURNS!');setTimeout(()=>{_ramiNextAiOrPlayer();G.busy=false;},1400);return;}}
  // Play melds
...
// ─── Init ─────────────────────────────────────────────────────
function initRamiGame(rounds,difficulty='medium',playerCount=2){
  const target=window.ramiTargetScore||101;
  // Build player list
  let players;
  if(playerCount===4){
    players=[
      {id:0,name:'YOU',  score:0,hand:[],laidMelds:[],isAI:false},
      {id:1,name:'LAYLA',score:0,hand:[],laidMelds:[],isAI:true},
      {id:2,name:'SAMI', score:0,hand:[],laidMelds:[],isAI:true},
      {id:3,name:'KARIM',score:0,hand:[],laidMelds:[],isAI:true},
    ];
  } else {
    players=[
      {id:0,name:'YOU', score:0,hand:[],laidMelds:[],isAI:false},
      {id:1,name:'SAMI',score:0,hand:[],laidMelds:[],isAI:true},
    ];
  }
  // Build hasOpened map
  ...
  G={gameType:'rami',totalRounds:rounds,round:0,difficulty,ramiTarget:target,ramiOpeningMin:window.ramiOpeningMin||51,ramiPlayerCount:playerCount,
    players,
  ...
  setTimeout(()=>{
    ...
    G.round=1;ramiUpdateRoundPill();renderRamiGame();ramiSetMsg('YOUR TURN — DRAW A CARD');
  },150);
}
...
function ramiUpdateRoundPill(){
  ...
  const lbl=document.getElementById('hdrRoundLabel');if(lbl)lbl.textContent='SCORE';
}
...
<div style="display:flex;flex-direction:column;flex:1;min-height:0;gap:clamp(4px,0.8vh,8px);">
  ...
  <div style="display:flex;justify-content:center;flex-shrink:0;">
    <div style="display:inline-flex;align-items:center;gap:clamp(6px,1vw,10px);padding:clamp(4px,0.8vh,6px) clamp(12px,2vw,18px);background:rgba(0,0,0,0.85);border:3px solid #8B6914;box-shadow:2px 2px 0 #000;">
      ...
      <span style="font-family:'Press Start 2P',monospace;font-size:clamp(7px,1vw,10px);color:#C9A84C;">SAMI</span>
...
<div style="position:relative;display:flex;align-items:center;justify-content:center;flex-shrink:0;padding:clamp(6px,1.2vh,10px) 0;margin:clamp(4px,0.8vh,8px) 0;">
  ...
  <div style="position:absolute;left:clamp(8px,1.8vw,22px);top:50%;transform:translateY(-50%);display:flex;flex-direction:column;align-items:flex-start;gap:clamp(3px,0.6vh,5px);">
    ...
    <div id="ramiDrawPile" onclick="ramiPlayerDraw()" style="position:relative;width:clamp(76px,10.5vw,116px);height:clamp(72px,10vw,94px);cursor:pointer;transition:transform 0.12s;">
...
function ramiNumpadConfirm(){
  const val=parseInt(_ramiNumpadVal)||101;
  const final=Math.max(1,val); // any positive number
...
function renderRamiGame(){
  ...
  const rami=G.rami,p=G.players[0],ai=G.players[1];
  const myTurn=rami.currentTurn===0&&!G.busy;
  ...
  if(pst){pst.textContent=rami.hasOpened[0]?'OPENED':'NOT OPENED';pst.style.color=rami.hasOpened[0]?'#4CAF50':'#666';}
  if(ast){ast.textContent=rami.hasOpened[1]?'OPENED':'NOT OPENED';ast.style.color=rami.hasOpened[1]?'#4CAF50':'#666';}
  const be=document.getElementById('ramiBanDisplay');
  if(be){if(rami.playerBanRounds>0){be.textContent='BANNED '+rami.playerBanRounds+'T';be.style.display='';}else be.style.display='none';}
  const pd=document.getElementById('ramiPlayerTurnDot'),ad=document.getElementById('ramiAiTurnDot');
  if(pd)pd.style.display=rami.currentTurn===0?'':'none';if(ad)ad.style.display=rami.currentTurn===1?'':'none';
  ...
  // AI melds — top area, sorted, centered
  const amel=document.getElementById('ramiAiMelds');
  if(amel){
    amel.innerHTML='';
    rami.tableMelds.filter(m=>m.owner===1).forEach(meld=>{
      const md=document.createElement('div');
      md.style.cssText='display:flex;gap:2px;align-items:center;padding:3px 6px;background:rgba(0,0,0,0.55);border:2px solid #FF6644;';
      ramiSortMeld(meld.cards).forEach(c=>md.appendChild(ramiSmallCard(c,false)));
      amel.appendChild(md);
    });
  }

  // Player melds — bottom area, sorted, centered
  ...
  if(pmel){
    ...
    rami.tableMelds.filter(m=>m.owner===0).forEach((meld,_mi)=>{
      // find real index in tableMelds
    ...
    // Also allow placing on AI melds — they appear in AI area but we handle via same logic
    // Show "no fit" hint
  ...
  // Also render AI melds as interactive when player has opened
  const amel2=document.getElementById('ramiAiMelds');
  if(amel2){
    const canInteract=myTurn&&rami.hasOpened[0]&&rami.playerPhase==='play';
    const selCard=rami.tableSelCard;
    amel2.innerHTML='';
    rami.tableMelds.filter(m=>m.owner===1).forEach(meld=>{
      ...
      ramiSortMeld(meld.cards).forEach(c=>md.appendChild(ramiSmallCard(c,c.r==='JK'&&hasJoker)));
      if(canPlace||hasJoker)md.addEventListener('click',()=>ramiPlayerPlaceOnTable(mi));
      amel2.appendChild(md);
    });
  }

  // Player hand with drag reorder
...
function ramiHandCardClick(card){
  ...
  // Also track for table placement when opened
  rami.tableSelCard=rami.hasOpened[0]?(same?null:card):null;
  sfxClick();renderRamiGame();
}

// ─── Place on table ───────────────────────────────────────────
function ramiPlayerPlaceOnTable(meldIndex){
  const rami=G.rami,p=G.players[0];
  ...
  if(p.hand.length===0){ramiEndRound(0);return;}
  ramiSetMsg('PLACED! PLACE MORE OR DISCARD');renderRamiGame();
}

// ─── TAHBAT ───────────────────────────────────────────────────
function ramiOpenTahbat(){
  const rami=G.rami;rami.tahbatOpen=true;rami.tahbatSlots=[[]];rami.tahbatActive=0; // start with 1 slot
  ...
  if(h)h.textContent=rami.hasOpened[0]?'ADD CARDS TO SLOTS — LAY NEW MELDS':'NEED '+openMin+'+ PTS TOTAL TO OPEN';
...
function ramiRenderTahbatHand(){
  ...
  // Track which card instances are used (by index in hand, not by rank+suit)
  const usedIndices=new Set();
  const hand=G.players[0].hand;
  G.rami.tahbatSlots.flat().forEach(slotCard=>{
    // Find first matching card in hand that isn't already used
...
function ramiCheckTahbatConfirm(){
  ...
  const openMin=G.ramiOpeningMin||51;
  if(!rami.hasOpened[0]&&score<openMin){btn.style.display='none';if(h)h.textContent='NEED '+openMin+'+ PTS — CURRENTLY '+score+' PTS';return;}
  btn.style.display='';
...
function ramiTahbatConfirm(){
  ...
  if(!rami.hasOpened[0]){
    // Find highest opening score so far
    ...
    // Must beat highest opening by at least 1
    ...
    // Success - record opening
    rami.hasOpened[0]=true;
    rami.openingScores[0]=total;
  }
  
  slots.forEach(slot=>{rami.tableMelds.push({owner:0,cards:ramiSortMeld([...slot])});if(!p.laidMelds)p.laidMelds=[];p.laidMelds.push([...slot]);slot.map(c=>c.r+c.s).forEach(k=>{const i=p.hand.findIndex(c=>c.r+c.s===k);if(i>=0)p.hand.splice(i,1);});});
...
function ramiRenderActionBtns(myTurn){
  ...
  const isMyTurn=rami.currentTurn===0;
  ...
  if(rami.selectedHandCard){
    ...
    btn.addEventListener('click',ramiPlayerDiscard);
    c.appendChild(btn);
  }
}
...
function ramiPlayerDraw(){
  if(G.busy||G.rami.currentTurn!==0||G.rami.playerPhase!=='draw')return;
  const ban=G.rami.banRounds&&G.rami.banRounds[0]||0;
  if(ban>0){G.rami.banRounds[0]--;renderRamiGame();ramiSetMsg('BANNED — SKIPPING ('+G.rami.banRounds[0]+' LEFT)');setTimeout(()=>{_ramiNextAiOrPlayer();},1100);return;}
  const rami=G.rami;
  if(rami.drawPile.length===0){if(rami.discardPile.length<=1){ramiSetMsg('DRAW PILE EMPTY!');return;}const last=rami.discardPile.pop();rami.drawPile=shuffle(rami.discardPile);rami.discardPile=last?[last]:[];}
  if(rami.drawPile.length>0){G.players[0].hand.push(rami.drawPile.pop());sfxCard();}
  ramiAfterDraw();
}
function ramiPlayerDrawDiscard(){
  if(G.busy||G.rami.currentTurn!==0||G.rami.playerPhase!=='draw')return;
  const rami=G.rami;if(!rami.discardPile.length)return;
  if(rami.hasOpened[0]){const card=rami.discardPile.pop();G.players[0].hand.push(card);sfxCard();ramiAfterDraw();return;}
  const card=rami.discardPile.pop();G.players[0].hand.push(card);
  const mm=bestMeldSet(G.players[0].hand);
  if(!canOpen(mm)){
    G.players[0].hand=G.players[0].hand.filter(c=>!(c.r===card.r&&c.s===card.s));
    rami.discardPile.push(card);
    if(!rami.banRounds)rami.banRounds={};rami.banRounds[0]=2; // player ban stored in same map
    sfxLose();showToast('TOOK DISCARD BUT CANNOT OPEN — 2 TURN BAN!');
    ramiSetMsg('CANNOT OPEN — 2 TURN BAN!');renderRamiGame();
    setTimeout(()=>{_ramiNextAiOrPlayer();},1700);return;
  }
  sfxCard();rami.playerPhase='play';ramiSetMsg('TOOK DISCARD — OPEN NOW! USE TAHBAT');renderRamiGame();
}
function ramiAfterDraw(){
  const rami=G.rami,p=G.players[0];
  rami.playerPhase='play';rami.selectedHandCard=null;rami.tableSelCard=null;
  if(!rami.hasOpened[0]){const mm=bestMeldSet(p.hand);if(canOpen(mm)){ramiSetMsg('YOU CAN OPEN! TAP TAHBAT');}else{ramiSetMsg('TAP A CARD TO DISCARD OR USE TAHBAT');}}
  else{ramiSetMsg('TAP A CARD TO DISCARD OR USE TAHBAT');}
  renderRamiGame();
}
function ramiPlayerDiscard(){
  if(G.busy||G.rami.currentTurn!==0)return;
  const rami=G.rami,p=G.players[0],card=rami.selectedHandCard;if(!card)return;
  // Rule: can't discard a card that fits a table meld unless you have a duplicate
  ...
  const i=p.hand.findIndex(c=>c.r===card.r&&c.s===card.s);if(i>=0)p.hand.splice(i,1);
  rami.discardPile.push(card);rami.selectedHandCard=null;rami.tableSelCard=null;sfxCard();
  if(p.hand.length===0){ramiEndRound(0);return;}
  _ramiNextAiOrPlayer();
}
...
function ramiEndRound(wi){
  ...
  // Penalise all losers (everyone except winner)
  let penMsg='';
  G.players.forEach(p=>{
    if(p.id!==wi){const pen=calculateRamiScore(p);p.score+=pen;penMsg+=p.name+' +'+pen+' ';}
  });
  showToast(wn.name+' WINS ROUND '+G.round+'! — '+penMsg.trim());
  if(wi===0){sfxWin();launchConfetti();}else sfxLose();
  const target=G.ramiTarget||101;
  const gameOver=G.players.some(p=>p.score>=target)||G.round>=G.totalRounds;
  setTimeout(()=>{G.busy=false;if(gameOver){ramiEndGame();return;}ramiNextRound();},2200);
}
function ramiNextRound(){
  ...
  G.players[0].hand=ramiSortHand(G.players[0].hand);
  const hasOpened={},banRounds={},openingScores={};
  G.players.forEach(p=>{hasOpened[p.id]=false;openingScores[p.id]=0;if(p.isAI)banRounds[p.id]=0;});
  const startTurn=G.round%n; // rotate starting player each round
  G.rami={drawPile:deck,discardPile:[deck.pop()],tableMelds:[],hasOpened,banRounds,openingScores,
    ...
    tahbatOpen:false,tahbatSlots:[[]],tahbatActive:0};
  G.busy=false;ramiUpdateRoundPill();renderRamiGame();
  if(startTurn===0){ramiSetMsg('ROUND '+G.round+' — YOUR TURN — DRAW A CARD');}
  else{ramiSetMsg('ROUND '+G.round+' — '+G.players[startTurn].name+' GOES FIRST...');setTimeout(ramiAiTurn,1100);}
}
function ramiEndGame(){
  const sv=[...G.players].sort((a,b)=>a.score-b.score),wn=sv[0];sfxWin();launchConfetti();
  ...
  authAwardXp(wn.id===0);setTimeout(()=>goTo('gameover'),1200);
}
...
function create4PRamiUI(){
  ...
  ui.innerHTML=`
<style>
#r4p-wrap{width:100%;height:100%;display:grid;grid-template-columns:clamp(60px,9vw,110px) 1fr clamp(60px,9vw,110px);grid-template-rows:auto 1fr auto;box-sizing:border-box;}
</style>
<div id="r4p-wrap">
...
<div style="grid-column:1/4;grid-row:1;display:flex;flex-direction:column;align-items:center;padding:2px 4px;gap:1px;background:rgba(0,0,0,0.1);">
  ...
  <div style="display:inline-flex;align-items:center;gap:6px;padding:2px 12px;background:rgba(0,0,0,0.85);border:2px solid #8B6914;">
    ...
    <span style="font-family:'Press Start 2P',monospace;font-size:clamp(7px,0.9vw,10px);color:#C9A84C;">SAMI</span>
  ...
  <!-- SAMI melds -->
  <div id="ramiP2Melds" style="display:flex;flex-wrap:wrap;gap:3px;justify-content:center;padding:1px 8px;min-height:clamp(32px,4.5vh,50px);max-height:clamp(50px,7vh,72px);overflow:hidden;width:100%;"></div>
</div>
...
<div style="grid-column:1;grid-row:2;display:flex;flex-direction:row;align-items:stretch;overflow:hidden;padding:2px 0;">
  ...
  <div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1;padding:4px 2px;overflow:hidden;">
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:3px 5px;background:rgba(0,0,0,0.85);border:2px solid #8B6914;">
      ...
      <span style="font-family:'Press Start 2P',monospace;font-size:clamp(5px,0.65vw,8px);color:#C9A84C;">LAYLA</span>
    ...
    <div id="ramiP1Melds" style="display:flex;flex-direction:column;gap:2px;align-items:center;overflow:hidden;flex:1;width:100%;padding:0 1px;"></div>
  </div>
</div>
...
<div style="grid-column:2;grid-row:2;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;padding:4px 8px;overflow:hidden;">
  ...
  <div style="display:flex;align-items:flex-end;gap:clamp(10px,2vw,22px);">
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <div id="ramiDrawPile" onclick="ramiPlayerDraw()" style="position:relative;width:clamp(58px,8vw,78px);height:clamp(66px,9vw,88px);cursor:pointer;transition:transform 0.1s;">
    ...
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <div id="ramiDiscardPile" onclick="ramiPlayerDrawDiscard()" style="width:clamp(52px,7.2vw,72px);height:clamp(72px,10vw,96px);background:#1a0800;border:3px solid #8B6914;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;cursor:pointer;box-shadow:3px 3px 0 #000;transition:border-color 0.12s,transform 0.1s;">
...
<div style="grid-column:3;grid-row:2;display:flex;flex-direction:row-reverse;align-items:stretch;overflow:hidden;padding:2px 0;">
  ...
  <div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1;padding:4px 2px;overflow:hidden;">
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:3px 5px;background:rgba(0,0,0,0.85);border:2px solid #8B6914;">
      ...
      <span style="font-family:'Press Start 2P',monospace;font-size:clamp(5px,0.65vw,8px);color:#C9A84C;">KARIM</span>
    ...
    <div id="ramiP3Melds" style="display:flex;flex-direction:column;gap:2px;align-items:center;overflow:hidden;flex:1;width:100%;padding:0 1px;"></div>
  </div>
</div>
...
<div style="grid-column:1/4;grid-row:3;display:flex;flex-direction:column;align-items:center;flex-shrink:0;padding:0 0 2px;">
  ...
  <div id="ramiPlayerMelds" style="display:flex;flex-wrap:wrap;gap:3px;justify-content:center;max-height:clamp(50px,7.5vh,76px);overflow:hidden;padding:2px 4px;width:100%;margin-bottom:3px;"></div>
...
function render4PRamiGame(){
  ...
  G.players.forEach((pl,i)=>{
    const def=pDefs[i];
    const sc=document.getElementById(def.scoreId);if(sc)sc.textContent=pl.score;
